...
queue.prototype.readJob = function(q, filename, callback){
  var self = this;
  var directory = self.basePath + path.sep + 'queues' + path.sep + q;
  fs.readFile(directory + path.sep + filename + '.json', function(err, contents){
    var payload;
...
queue.prototype.readTimestampJob = function(timestamp, filename, callback){
  var self = this;
  var directory = self.basePath + path.sep + 'delayed' + path.sep + timestamp;
  fs.readFile(directory + path.sep + filename + '.json', function(err, contents){
    var payload;
